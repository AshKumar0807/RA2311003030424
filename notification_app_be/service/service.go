...
func (s *NotificationService) Send(req domain.SendNotificationRequest) (*domain.Notification, error) {
	...
	if req.JobID == "" { return nil, fmt.Errorf("job_id required") }
	if req.RecipientID == "" { return nil, fmt.Errorf("recipient_id required") }
	if req.Body == "" { return nil, fmt.Errorf("body required") }
	valid := map[domain.Channel]bool{domain.ChannelEmail: true, domain.ChannelSMS: true, domain.ChannelPush: true}
	if !valid[req.Channel] { return nil, fmt.Errorf("channel must be email, sms, or push") }
	...
	sentAt := now; n.SentAt = &sentAt
	s.mu.Lock(); s.notifications[n.ID] = n; s.mu.Unlock()
...
func (s *NotificationService) GetNotification(id string) (*domain.Notification, error) {
	...
	if !ok { return nil, fmt.Errorf("notification %q not found", id) }
	return n, nil
}
...
func (s *NotificationService) ListNotifications() []*domain.Notification {
	...
	for _, n := range s.notifications { out = append(out, n) }
	return out
}
