...
func (s *Store) GetVehicle(id string) (*domain.Vehicle, error) {
	...
	v, ok := s.vehicles[id]
	if !ok { return nil, fmt.Errorf("vehicle %q not found", id) }
	return v, nil
}
...
func (s *Store) ListVehicles() []*domain.Vehicle {
	...
	for _, v := range s.vehicles { out = append(out, v) }
	return out
}
...
func (s *Store) GetJob(id string) (*domain.MaintenanceJob, error) {
	s.mu.RLock(); defer s.mu.RUnlock()
	job, ok := s.jobs[id]
	if !ok { return nil, fmt.Errorf("job %q not found", id) }
	return job, nil
}
...
func (s *Store) ListJobsForVehicle(vehicleID string) []*domain.MaintenanceJob {
	...
	for _, j := range s.jobs { if j.VehicleID == vehicleID { out = append(out, j) } }
	return out
}
...
func (s *Store) UpdateJobStatus(id string, req domain.UpdateJobStatusRequest) (*domain.MaintenanceJob, error) {
	...
	job, ok := s.jobs[id]
	if !ok { return nil, fmt.Errorf("job %q not found", id) }
	job.Status = req.Status; job.UpdatedAt = time.Now().UTC()
	if req.TechnicianName != "" { job.TechnicianName = req.TechnicianName }
	if req.Notes != "" { job.Notes = req.Notes }
	if req.Status == domain.StatusCompleted { now := time.Now().UTC(); job.CompletedAt = &now }
